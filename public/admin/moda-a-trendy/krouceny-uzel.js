...
document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("http://localhost:8080/api/html/fashion_trends/node");
...
saveButton.addEventListener("click", async () => {
    ...
    await sendHTML(div.innerHTML, "http://localhost:8080/api/html/fashion_trends/node");
    await sentImage();
    location.reload();
})
...
async function firstBlock(html){
    ...
    const imagesCMS = blocks[0].querySelector("img")
    const relativeSrc = html.querySelector("img").getAttribute('data-relative-src');

        if(relativeSrc){
            imagesCMS.src = "../" + relativeSrc
            imagesCMS.setAttribute("data-relative-src", "../" + relativeSrc)
        } else {
            imagesCMS.src = html.querySelector("img").src;
        }
