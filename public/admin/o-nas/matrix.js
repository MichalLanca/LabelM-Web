...
document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("http://localhost:8080/api/html/about_us/matrix");
...
saveButton.addEventListener("click", async () => {
    ...
    await sendHTML(div.innerHTML, "http://localhost:8080/api/html/about_us/matrix");
    await sentImage();
    location.reload();
})
...
async function secondBlock(html){
    ...
    const header = html.querySelectorAll("h1, h2, h3, h4, h5, h6");
    const inputs = blocks[1].querySelectorAll("input[type='text']")
    const boldText = html.querySelector("b")
    const vimeo = html.querySelector("iframe")
    const cmsVideo = blocks[1].querySelector("iframe")

    let bold = false
    let i = 0
    text.forEach((t) => {
        const hasBold = paragpraphs[i].querySelector("b") !== null;
        if(bold){
            t.textContent = removeExtraSpaces(boldText.textContent)
            bold = false
        }
        else if(hasBold){
            t.textContent = removeExtraSpaces(paragpraphs[i].firstChild.textContent)
            bold = true
            i++
        } else {
            t.textContent = removeExtraSpaces(paragpraphs[i].textContent)
            i++
        }
    })
...
function updateSecondBlock(oldHtml, cmsHtml, updatedImages){
    updateImagesSource(oldHtml, updatedImages)
    const oldH2 = oldHtml.querySelectorAll("h1, h2, h3, h4, h5, h6");
    ...
    let bold = false
    textarea.forEach((t) => {
        const hasBold = p[i].querySelector("b") !== null;
        if(bold){           
            p[i].querySelector("b").textContent = t.textContent
            i++
            bold = false
        } else if(hasBold && bold === false){
            p[i].firstChild.textContent = t.textContent
            bold = true
        } else {
            p[i].textContent = t.textContent
            i++
        }
    })
