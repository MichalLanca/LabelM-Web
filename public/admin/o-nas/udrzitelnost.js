...
document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("http://localhost:8080/api/html/about_us/sustainability");
...
saveButton.addEventListener("click", async () => {
    ...
    await sendHTML(div.innerHTML, "http://localhost:8080/api/html/about_us/sustainability");
    await sentImage();
    location.reload();
})
...
async function secondBlock(html){
    ...
    const heading = html.querySelector("h3")
    const cmsH = blocks[1].querySelector("input[type='text']")
    ...
    cmsH.value = heading.textContent
...
function updateSecondBlock(oldHtml, cmsHtml, updatedImages){
    ...
    const oldH = oldHtml.querySelector("h3")
    const newH = cmsHtml.querySelector("input[type='text']")

    oldP.forEach((p, index) => {
        p.textContent = newP[index].value;
    })

    oldH.textContent = newH.value
