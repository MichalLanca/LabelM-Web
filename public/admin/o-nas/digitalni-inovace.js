...
document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("http://localhost:8080/api/html/about_us/digital_inovation");
...
saveButton.addEventListener("click", async () => {
    ...
    await sendHTML(div.innerHTML, "http://localhost:8080/api/html/about_us/digital_inovation");
    await sentImage();
    location.reload();
})
...
async function thirdBlock(html){
    ...
    paragpraphs.forEach((p, index) => {
            textAreas[index].textContent = removeExtraSpaces(p.textContent);
    })  
